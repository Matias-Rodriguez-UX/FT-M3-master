...
async function problemA() {
  ...
  // AsyncAwait version
  const poems = ['poem-two/stanza-01.txt', 'poem-two/stanza-02.txt'].map(file => promisifiedReadFile(file))
  const poem = await Promise.all(poems)
  poem.forEach(p => blue(p))
  console.log('done')
}
...
async function problemB() {
  ...
  // AsyncAwait version
  const proms = await filenames.map(file => promisifiedReadFile(file))
  const poems = await Promise.all(proms)
  poems.forEach(p => blue(p))
  console.log('done')
